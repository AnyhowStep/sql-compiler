...
TableIdentifierList2 ->
    TableIdentifier (%Comma TableIdentifier):* {% (data) => {
    const [first, more] = data;
    const arr = more
        .flat(1)
        .filter((x) => {
...
AlterServerStatement ->
    %ALTER %SERVER (Identifier | StringLiteral) %OPTIONS %OpenParentheses CreateServerOptions %CloseParentheses {% (data) => {
...
AlterTableAlterColumnDropDefault ->
    %ALTER %COLUMN:? ColumnIdentifier %DROP %DEFAULT {% (data) => {
...
AlterTableDropForeignKey ->
    %DROP %FOREIGN %KEY ColumnIdentifier {% (data) => {
...
AnalyzeTableStatement ->
    %ANALYZE (%NO_WRITE_TO_BINLOG | %LOCAL):? (%TABLE | %TABLES) TableIdentifierList2 {% (data) => {
